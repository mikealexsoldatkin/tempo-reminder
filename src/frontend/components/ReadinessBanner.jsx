 ...
 */
export const ReadinessBanner = ({ settings, credentials, slack, trackedUsers, managers }) => {
  ...
  const blockers = [
    !credentials?.tempoToken?.isSet &&
      'The Tempo API token is not set — worklogs can’t be read. See the “Access” tab.',
    !credentials?.slackBotToken?.isSet &&
      'Slack is not connected — nothing can be delivered. Press “Connect Slack” on the “Access” tab.',
    ...
    credentials?.slackBotToken?.isSet &&
      ...
      `Slack rejected the connection (${slack.connection.revokedError}) — nothing is being delivered. Reconnect on the “Access” tab.`,
    users.length === 0 &&
