...
- Ещё параметры: пропуск выходных, времена запуска двух рассылок и три шаблона сообщений
  (сотруднику, менеджеру-дайджест, менеджеру «все отчитались») с плейсхолдерами `{name}`, `{from}`,
  `{to}`, `{days}`, `{missing}`, `{missingCount}`, `{count}`, `{list}`.
