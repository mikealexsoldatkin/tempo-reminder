...
- Ещё параметры: пропуск выходных, **учёт календаря праздников** (см. вкладку «Holidays»), времена
  запуска двух рассылок и три шаблона сообщений (сотруднику, менеджеру-дайджест, менеджеру «все
  отчитались») с плейсхолдерами `{name}`, `{from}`, `{to}`, `{days}`, `{missing}`, `{missingCount}`,
  `{count}`, `{list}`.

### 4. Вкладка «Holidays»
Календарь праздников: праздник — не рабочий день, поэтому в окно проверки он не попадает и планового
прогона в этот день нет. Управляется галочкой **«Take the holiday calendar into account»** на вкладке
«Check parameters» — с выключенной календарь становится просто справочным списком.
